...
However, because we know by observing the residuals that there is a spike in the 12th obvservation, we are also going to test using a p or q values equal to 12 to see if we get lower values than the 3 0 3 model.
...
```{r}
#arima
fcst=forecast(final.model, method="arima", h=24)
plot(fcst)
```
  

  g) *Use the function getrmse to compute the test set RMSE of some of the models you have already fiited. Which is the one minimizing it? Use the last year of observations (12 observations for monthly data, 4 observations for quarterly data) as the test set.*
  
```{r}
#To be honest i cant find that function. We may have to load it.

# We also have to do the data separation to leave the last year as a Test portion. (last 12 obs.)
